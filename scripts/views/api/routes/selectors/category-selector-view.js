...
], function($, _, Backbone, Template, Registry, Route, ErrorView, NameSelectorView) {
			...
			Route.fetchCategories({
				...
				success: function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						categories.push({
							name: data[i].toTitleCase()
						});
					}

					// set attributes
					//
					self.collection = new Backbone.Collection(categories);
